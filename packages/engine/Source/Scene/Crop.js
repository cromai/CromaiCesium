 ...
 * properties are set by calling {@link CropCollection#add}. Do not call the constructor directly.
 * </div>
 * A viewport-aligned image positioned in the 3D scene, that is created
 * and rendered using a {@link CropCollection}.
 * <br /><br />
 * <div align='center'>
 * <img src='Images/Crop.png' width='400' height='300' /><br />
 * Example billboards
 * </div>
 *
 * @alias Crop
 *
 * @performance Reading a property, e.g., {@link Crop#show}, is constant time.
 * Assigning to a property is constant time but results in
 * CPU to GPU traffic when {@link CropCollection#update} is called.  The per-billboard traffic is
 * the same regardless of how many properties were updated.  If most billboards in a collection need to be
 * updated, it may be more efficient to clear the collection with {@link CropCollection#removeAll}
 ...
 *
 * @see CropCollection
 * @see CropCollection#add
 * @see Label
 *
 * @internalConstructor
 * @class
 *
 * @demo {@link https://sandcastle.cesium.com/index.html?src=Crops.html|Cesium Sandcastle Crop Demo}
 */
function Crop(options, billboardCollection) {
  ...
  this._pickColor = Color.clone(defaultValue(options.pickColor, Color.WHITE));
  ...
  this._dirty = false;
  this._index = -1; //Used only by CropCollection
  this._batchIndex = undefined; // Used only by Vector3DTilePoints and CropCollection
...
const SHOW_INDEX = (Crop.SHOW_INDEX = 0);
const POSITION_INDEX = (Crop.POSITION_INDEX = 1);
const PIXEL_OFFSET_INDEX = (Crop.PIXEL_OFFSET_INDEX = 2);
const EYE_OFFSET_INDEX = (Crop.EYE_OFFSET_INDEX = 3);
const HORIZONTAL_ORIGIN_INDEX = (Crop.HORIZONTAL_ORIGIN_INDEX = 4);
const VERTICAL_ORIGIN_INDEX = (Crop.VERTICAL_ORIGIN_INDEX = 5);
const SCALE_INDEX = (Crop.SCALE_INDEX = 6);
const IMAGE_INDEX_INDEX = (Crop.IMAGE_INDEX_INDEX = 7);
const COLOR_INDEX = (Crop.COLOR_INDEX = 8);
const ROTATION_INDEX = (Crop.ROTATION_INDEX = 9);
const ALIGNED_AXIS_INDEX = (Crop.ALIGNED_AXIS_INDEX = 10);
const SCALE_BY_DISTANCE_INDEX = (Crop.SCALE_BY_DISTANCE_INDEX = 11);
const TRANSLUCENCY_BY_DISTANCE_INDEX = (Crop.TRANSLUCENCY_BY_DISTANCE_INDEX = 12);
const PIXEL_OFFSET_SCALE_BY_DISTANCE_INDEX = (Crop.PIXEL_OFFSET_SCALE_BY_DISTANCE_INDEX = 13);
const DISTANCE_DISPLAY_CONDITION = (Crop.DISTANCE_DISPLAY_CONDITION = 14);
const DISABLE_DEPTH_DISTANCE = (Crop.DISABLE_DEPTH_DISTANCE = 15);
Crop.TEXTURE_COORDINATE_BOUNDS = 16;
const SDF_INDEX = (Crop.SDF_INDEX = 17);
Crop.NUMBER_OF_PROPERTIES = 18;

function makeDirty(billboard, propertyChanged) {
  const billboardCollection = billboard._billboardCollection;
  if (defined(billboardCollection)) {
    billboardCollection._updateCrop(billboard, propertyChanged);
    billboard._dirty = true;
  }
}

Object.defineProperties(Crop.prototype, {
  /**
   * Determines if this billboard will be shown.  Use this to hide or show a billboard, instead
   * of removing it and re-adding it to the collection.
   * @memberof Crop.prototype
   * @type {boolean}
   * @default true
   */
  show: {
   ...
   * <td align='center'><code>default</code><br/><img src='Images/Crop.setPixelOffset.default.png' width='250' height='188' /></td>
   * <td align='center'><code>b.pixeloffset = new Cartesian2(50, 25);</code><br/><img src='Images/Crop.setPixelOffset.x50y-25.png' width='250' height='188' /></td>
   * </tr></table>
  ...
  /**
   * Gets or sets near and far scaling properties of a Crop based on the billboard's distance from the camera.
  ...
  /**
   * Gets or sets near and far translucency properties of a Crop based on the billboard's distance from the camera.
  ...
  /**
   * Gets or sets near and far pixel offset scaling properties of a Crop based on the billboard's distance from the camera.
   ...
   * <div align='center'>
   * <table border='0' cellpadding='5'><tr>
   * <td align='center'><img src='Images/Crop.setEyeOffset.one.png' width='250' height='188' /></td>
   * <td align='center'><img src='Images/Crop.setEyeOffset.two.png' width='250' height='188' /></td>
   * </tr></table>
   ...
   * <br /><br />
   * <div align='center'>
   * <img src='Images/Crop.setHorizontalOrigin.png' width='648' height='196' /><br />
   * </div>
   * @memberof Crop.prototype
   ...
   * <br /><br />
   * <div align='center'>
   * <img src='Images/Crop.setVerticalOrigin.png' width='695' height='175' /><br />
   * </div>
   * @memberof Crop.prototype
   * @type {VerticalOrigin}
   * @example
   ...
   * <br /><br />
   * <div align='center'>
   * <img src='Images/Crop.setScale.png' width='400' height='300' /><br/>
   ...
   * <div align='center'>
   * <table border='0' cellpadding='5'><tr>
   * <td align='center'><code>default</code><br/><img src='Images/Crop.setColor.Alpha255.png' width='250' height='188' /></td>
   * <td align='center'><code>alpha : 0.5</code><br/><img src='Images/Crop.setColor.Alpha127.png' width='250' height='188' /></td>
   * </tr></table>
   * </div>
   * <br />
  ...
  /**
   * The outline color of this Crop.  Effective only for SDF billboards like Label glyphs.
   * @memberof Crop.prototype
   * @type {Color}
   * @private
   */
  ...
  /**
   * The outline width of this Crop in pixels.  Effective only for SDF billboards like Label glyphs.
   * @memberof Crop.prototype
   * @type {number}
   * @private
   */
...
Crop.prototype.getPickId = function (context) {
  if (!defined(this._pickId)) {
...
Crop.prototype._updateClamping = function () {
  Crop._updateClamping(this._billboardCollection, this);
};

const scratchCartographic = new Cartographic();
const scratchPosition = new Cartesian3();

Crop._updateClamping = function (collection, owner) {
...
Crop.prototype._loadImage = function () {
 ...
 * </p>
 * <p>
 * To load an image from a URL, setting the {@link Crop#image} property is more convenient.
 * </p>
 *
 * @param {string} id The id of the image.  This can be any string that uniquely identifies the image.
 * @param {HTMLImageElement|HTMLCanvasElement|string|Resource|Crop.CreateImageCallback} image The image to load.  This parameter
 ...
 */
Crop.prototype.setImage = function (id, image) {
  //>>includeStart('debug', pragmas.debug);
  if (!defined(id)) {
 ...
 */
Crop.prototype.setImageSubRegion = function (id, subRegion) {
  //>>includeStart('debug', pragmas.debug);
  if (!defined(id)) {
    throw new DeveloperError("id is required.");
  }
...
Crop.prototype._setTranslate = function (value) {
  //>>includeStart('debug', pragmas.debug);
  if (!defined(value)) {
    throw new DeveloperError("value is required.");
  }
...
Crop.prototype._getActualPosition = function () {
  return defined(this._clampedPosition)
    ? this._clampedPosition
    : this._actualPosition;
};

Crop.prototype._setActualPosition = function (value) {
...
Crop._computeActualPosition = function (
  billboard,
  position,
  frameState,
  modelMatrix
...
// This function is basically a stripped-down JavaScript version of CropCollectionVS.glsl
Crop._computeScreenSpacePosition = function (
  modelMatrix,
  position,
  eyeOffset,
  pixelOffset,
 ...
 *
 * @exception {DeveloperError} Crop must be in a collection.
 *
 * @example
 * console.log(b.computeScreenSpacePosition(scene).toString());
 *
 * @see Crop#eyeOffset
 * @see Crop#pixelOffset
 */
Crop.prototype.computeScreenSpacePosition = function (scene, result) {
  ...
  //>>includeStart('debug', pragmas.debug);
  if (!defined(billboardCollection)) {
    throw new DeveloperError("Crop must be in a collection.  Was it removed?");
  }
  if (!defined(scene)) {
  ...
  const windowCoordinates = Crop._computeScreenSpacePosition(
    modelMatrix,
    position,
    this._eyeOffset,
    scratchPixelOffset,
 ...
 * @param {Crop} billboard The billboard to get the screen space bounding box for.
 ...
 * @private
 */
Crop.getScreenSpaceBoundingBox = function (
  billboard,
  screenSpacePosition,
  result
) {
...
/**
 * Determines if this billboard equals another billboard.  Crops are equal if all their properties
 * are equal.  Crops in different collections can be equal.
 *
 * @param {Crop} other The billboard to compare for equality.
 * @returns {boolean} <code>true</code> if the billboards are equal; otherwise, <code>false</code>.
 */
Crop.prototype.equals = function (other) {
  return (
    this === other ||
    (defined(other) &&
      this._id === other._id &&
...
Crop.prototype._destroy = function () {
 ...
 * @callback Crop.CreateImageCallback

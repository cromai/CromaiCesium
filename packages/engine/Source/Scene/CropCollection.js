...
import CropCollectionFS from "../Shaders/CropCollectionFS.js";
import CropCollectionVS from "../Shaders/CropCollectionVS.js";
...
const SHOW_INDEX = Crop.SHOW_INDEX;
const POSITION_INDEX = Crop.POSITION_INDEX;
const PIXEL_OFFSET_INDEX = Crop.PIXEL_OFFSET_INDEX;
const EYE_OFFSET_INDEX = Crop.EYE_OFFSET_INDEX;
const HORIZONTAL_ORIGIN_INDEX = Crop.HORIZONTAL_ORIGIN_INDEX;
const VERTICAL_ORIGIN_INDEX = Crop.VERTICAL_ORIGIN_INDEX;
const SCALE_INDEX = Crop.SCALE_INDEX;
const IMAGE_INDEX_INDEX = Crop.IMAGE_INDEX_INDEX;
const COLOR_INDEX = Crop.COLOR_INDEX;
const ROTATION_INDEX = Crop.ROTATION_INDEX;
const ALIGNED_AXIS_INDEX = Crop.ALIGNED_AXIS_INDEX;
const SCALE_BY_DISTANCE_INDEX = Crop.SCALE_BY_DISTANCE_INDEX;
const TRANSLUCENCY_BY_DISTANCE_INDEX = Crop.TRANSLUCENCY_BY_DISTANCE_INDEX;
const PIXEL_OFFSET_SCALE_BY_DISTANCE_INDEX =
  Crop.PIXEL_OFFSET_SCALE_BY_DISTANCE_INDEX;
const DISTANCE_DISPLAY_CONDITION_INDEX = Crop.DISTANCE_DISPLAY_CONDITION;
const DISABLE_DEPTH_DISTANCE = Crop.DISABLE_DEPTH_DISTANCE;
const TEXTURE_COORDINATE_BOUNDS = Crop.TEXTURE_COORDINATE_BOUNDS;
const SDF_INDEX = Crop.SDF_INDEX;
const NUMBER_OF_PROPERTIES = Crop.NUMBER_OF_PROPERTIES;
...
/**
 * A renderable collection of billboards.  Crops are viewport-aligned
 ...
 * </div>
 * <br /><br />
 * Crops are added and removed from the collection using {@link CropCollection#add}
 * and {@link CropCollection#remove}.  Crops in a collection automatically share textures
 * for images with the same identifier.
 *
 * @alias CropCollection
 * @constructor
 *
 ...
 * const billboards = scene.primitives.add(new Cesium.CropCollection());
 * billboards.add({
 ...
 * });
 */
function CropCollection(options) {
   ...
   * This property is for debugging only; it is not for production use nor is it optimized.
   * <p>
   * Draws the texture atlas for this CropCollection as a fullscreen quad.
   * </p>
   *
   * @type {boolean}
   *
...
Object.defineProperties(CropCollection.prototype, {
  /**
   * Returns the number of billboards in this collection.  This is commonly used with
   * {@link CropCollection#get} to iterate over all the billboards
   * in the collection.
   * @memberof CropCollection.prototype
   * @type {number}
   */
  length: {
    get: function () {
...
function destroyCrops(billboards) {
  const length = billboards.length;
  for (let i = 0; i < length; ++i) {
    if (billboards[i]) {
 ...
 * @returns {Crop} The billboard that was added to the collection.
 *
 ...
 *
 * @see CropCollection#remove
 * @see CropCollection#removeAll
 */
CropCollection.prototype.add = function (options) {
  const billboard = new Crop(options, this);
 ...
 *
 * @param {Crop} billboard The billboard to remove.
 ...
 * {@link Crop#show} instead of removing and re-adding the billboard.
 *
 * @exception {DeveloperError} This object was destroyed, i.e., destroy() was called.
 *
 *
 ...
 * @see Crop#show
 */
CropCollection.prototype.remove = function (billboard) {
 ...
 * @see CropCollection#remove
 */
CropCollection.prototype.removeAll = function () {
  destroyCrops(this._billboards);
  this._billboards = [];
  this._billboardsToUpdate = [];
  this._billboardsToUpdateIndex = 0;
  this._billboardsRemoved = false;
...
function removeCrops(billboardCollection) {
  if (billboardCollection._billboardsRemoved) {
    ...
    for (let i = 0, j = 0; i < length; ++i) {
      ...
      if (defined(billboard)) {
        billboard._index = j++;
        newCrops.push(billboard);
      }
    }

    billboardCollection._billboards = newCrops;
  }
}

CropCollection.prototype._updateCrop = function (billboard, propertyChanged) {
  if (!billboard._dirty) {
 ...
 *
 * @param {Crop} [billboard] The billboard to check for.
 * @returns {boolean} true if this collection contains the billboard, false otherwise.
 *
 * @see CropCollection#get
 */
CropCollection.prototype.contains = function (billboard) {
 ...
 * {@link CropCollection#length} to iterate over all the billboards
 * in the collection.
 *
 * @param {number} index The zero-based index of the billboard.
 * @returns {Crop} The billboard at the specified index.
 *
 * @performance Expected constant time.  If billboards were removed from the collection and
 * {@link CropCollection#update} was not called, an implicit <code>O(n)</code>
 ...
 * }
 *
 * @see CropCollection#length
 */
CropCollection.prototype.get = function (index) {
...
CropCollection.prototype.computeNewBuffersUsage = function () {
...
function createVAF(
  ...
  const sizeInVertices = instanced ? numberOfCrops : 4 * numberOfCrops;
...
function recomputeActualPositions(
  ...
  for (let i = 0; i < length; ++i) {
    ...
    const actualPosition = Crop._computeActualPosition(
      billboard,
      position,
      frameState,
      modelMatrix
 ...
 */
CropCollection.prototype.update = function (frameState) {
  removeCrops(this);
  ...
  if (createVertexArray || (!picking && this.computeNewBuffersUsage())) {
    ...
    // Crops were modified, but none were added or removed.
  ...
  if (
    ...
    vsSource = CropCollectionVS;
    fsSource = CropCollectionFS;
 ...
 *
 * @see CropCollection#destroy
 */
CropCollection.prototype.isDestroyed = function () {
  return false;
};
 ...
 *
 * @see CropCollection#isDestroyed
 */
CropCollection.prototype.destroy = function () {
